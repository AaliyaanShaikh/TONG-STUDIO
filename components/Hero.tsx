  ...
  return (
    <div id="hero" ref={containerRef} className="relative h-[400vh] bg-black -mb-[1px]">
      ...
      <div className="sticky top-0 h-screen w-full flex flex-col overflow-hidden bg-black min-h-0">
        ...
        <div className="absolute inset-0 z-10 flex flex-col justify-between px-4 sm:px-6 md:px-20 pt-[max(4rem,env(safe-area-inset-top))] pb-12 pointer-events-none">
          <div className="relative flex-1 flex flex-col justify-center">
            {SLIDES.map((slide, i) => (
              ...
              >
                ...
                <h1 className="font-serif text-4xl sm:text-6xl md:text-8xl lg:text-9xl leading-[0.9] tracking-tighter mb-6 sm:mb-10">
                  <span className="block text-white">{slide.title}</span>
                  <span className="block italic text-stone-400">{slide.subtitle}</span>
                </h1>
                <p className="max-w-md text-white/95 text-sm sm:text-base font-medium leading-relaxed border-l-2 border-champagne-400 pl-4 sm:pl-6">
                  {slide.description}
                </p>
              </motion.div>
            ))}
          ...
          <div className="relative flex justify-between items-end text-[10px] uppercase tracking-widest min-h-[4rem] pointer-events-auto">
            {SLIDES.map((slide, i) => (
              ...
              >
                <div className="flex flex-wrap gap-4 sm:gap-8 font-bold text-white">
                ...
                <button
                  ...
                  className="rounded-full bg-white text-charcoal-900 px-2.5 py-1.5 sm:px-5 sm:py-2.5 text-[10px] sm:text-xs uppercase tracking-widest font-bold hover:bg-white/90 active:bg-white/80 transition-colors shadow-sm pointer-events-auto touch-manipulation min-h-[34px] sm:min-h-[44px]"
                >
                  Book a Session
                </button>
              </motion.div>
